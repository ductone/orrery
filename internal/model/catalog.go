...
var Catalog = []ModelSpec{
	...
	{ID: "together/deepseek-ai/DeepSeek-V4-Flash-0731", Family: DeepSeek, Tier: Efficient, Inputs: []Modality{Text}, ContextWindow: 1048576, MaxOutput: 128000, Pricing: Pricing{Input: .14, Output: .28, CacheRead: .03}, Effort: []Effort{EffortHigh}, Compat: Compat{MaxTokensField: "max_tokens", SupportsToolChoice: true, SupportsReasoningEffort: true, EffortWireMap: map[Effort]string{EffortHigh: "high"}, RequiresReasoningEcho: true, RequiresAssistantText: true, StreamIdleTimeout: 15 * time.Minute, SystemPromptStyle: SystemTopLevel}, EditDialect: HashlineContextual},
	...
	// Ramp Router reaches several vendors through one OpenAI Responses
	// endpoint. Limits, pricing, and effort levels below are transcribed from
	// GET /v1/models. Ramp also offers "minimal"/"max" effort, which orrery has
	// no level for, and "none", which is omitted deliberately: orrery drops the
	// reasoning field entirely for EffortNone, and the gateway would then apply
	// its own default_effort of "high" while the router priced the turn as
	// non-reasoning. Strict tool schemas are off for the Anthropic models:
	// strictifySchema makes every optional parameter a nullable union, and the
	// gateway rejects those requests over its 16-union-parameter limit.
	{ID: "ramp/claude-opus-5", Family: Anthropic, Tier: Frontier, Inputs: []Modality{Text, Image}, ContextWindow: 1000000, MaxOutput: 128000, Pricing: Pricing{Input: 5, Output: 25, CacheRead: .5, CacheWrite: 6.25}, Effort: []Effort{EffortLow, EffortMedium, EffortHigh, EffortXHigh}, Compat: Compat{MaxTokensField: "max_output_tokens", SupportsToolChoice: true, SupportsReasoningEffort: true, EffortWireMap: map[Effort]string{EffortLow: "low", EffortMedium: "medium", EffortHigh: "high", EffortXHigh: "xhigh"}, SystemPromptStyle: SystemTopLevel, StreamIdleTimeout: 15 * time.Minute}, EditDialect: HashlineXML},
	{ID: "ramp/claude-sonnet-5", Family: Anthropic, Tier: Efficient, Inputs: []Modality{Text, Image}, ContextWindow: 1000000, MaxOutput: 128000, Pricing: Pricing{Input: 2, Output: 10, CacheRead: .2, CacheWrite: 2.5}, Effort: []Effort{EffortLow, EffortMedium, EffortHigh, EffortXHigh}, Compat: Compat{MaxTokensField: "max_output_tokens", SupportsToolChoice: true, SupportsReasoningEffort: true, EffortWireMap: map[Effort]string{EffortLow: "low", EffortMedium: "medium", EffortHigh: "high", EffortXHigh: "xhigh"}, SystemPromptStyle: SystemTopLevel, StreamIdleTimeout: 10 * time.Minute}, EditDialect: HashlineXML},
	{ID: "ramp/gpt-5.6-sol", Family: OpenAI, Tier: Frontier, Inputs: []Modality{Text, Image}, ContextWindow: 1050000, MaxOutput: 128000, Pricing: Pricing{Input: 4, Output: 20, CacheRead: .4}, Effort: []Effort{EffortLow, EffortMedium, EffortHigh, EffortXHigh}, Compat: Compat{MaxTokensField: "max_output_tokens", SupportsToolChoice: true, SupportsReasoningEffort: true, EffortWireMap: map[Effort]string{EffortLow: "low", EffortMedium: "medium", EffortHigh: "high", EffortXHigh: "xhigh"}, SupportsStrictTools: true, SystemPromptStyle: SystemTopLevel, StreamIdleTimeout: 15 * time.Minute}, EditDialect: HashlineJSON},
